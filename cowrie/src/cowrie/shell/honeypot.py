...
class HoneyPotShell:
    ...
    def lineReceived(self, line: str) -> None:
        ...
        # Check for malicious patterns using Simple IDS
        ids = get_ids()
        session_id = getattr(self.protocol, 'transportId', 'unknown')
        username = getattr(self.protocol.user, 'username', b'unknown').decode('utf-8', errors='ignore') if hasattr(self.protocol, 'user') else 'unknown'
        is_malicious, severity, description = ids.check_command(line.encode(), session_id, username)
