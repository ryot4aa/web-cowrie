...
class HoneypotPasswordChecker:
    ...
    def checkUserPass(self, theusername: bytes, thepassword: bytes, ip: str) -> bool:
        ...
        # For safety: do not enable the random-auth behaviour unless explicitly allowed
        # by setting 'enable_auth_random = true' in the honeypot section. This prevents
        # accidental use of AuthRandom which permits authentication after random
        # attempts.
        if (
            authclass == "AuthRandom"
            and not CowrieConfig.getboolean("honeypot", "enable_auth_random", fallback=False)
        ):
            log.msg("AuthRandom requested but disabled by config, falling back to UserDB")
            authclass = "UserDB"
